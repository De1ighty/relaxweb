# 小胖庄园：上游架构适配与素材化美术升级设计

## 背景

上游 `main` 在 `89e6973` 将游戏厅拆成 `game-config.js`、`create-room.js`、
`room-chat.js`、`room-settlement.js`、`room-waiting.js` 等模块。小胖庄园分支仍把
...
2. 在 `game-config.js` 注册小胖庄园，元数据增加 `mode: "solo"` 与
...
闪光、水面波纹、钓鱼线、矿洞发光和小胖角色细节继续由本项目绘制，以避免地图
沦为素材拼贴。
