// Run with a local web server: NODE_PATH=<playwright node_modules> node tests/test_desktop.cjs
...
(async()=>{
 ...
 await page.getByRole('button',{name:'提示音：关',exact:true}).click();
 assert.equal(await page.evaluate(()=>localStorage.getItem('pokerTurnSound')),'on');
 await page.getByRole('button',{name:'提示音：开',exact:true}).click();
