...
const ICONS = { empty: "·", extra: "+2", stone: "🪨", coal: "◆", copper: "⬟", iron: "⬢", amethyst: "♦", star_gem: "✦" };

export function openMiningGame(root, run) {
  const layer = document.createElement("div"); layer.className = "estate-minigame mining-game";
  layer.innerHTML = `
    <div class="mine-cave-glow"></div>
    <div class="minigame-title"><b>小胖矿洞 · ${run.mine_name}</b><span>选择有矿脉痕迹的岩块</span></div>
  ...
  for (let index = 0; index < 25; index += 1) {
    ...
    cell.setAttribute("aria-label", `矿格 ${index + 1}`); cell.dataset.cell = index; cell.innerHTML = "<i></i><i></i>";
    cell.addEventListener("click", async () => {
      ...
      try {
        ...
        if (result.finished) showResult(result.result);
  ...
  function showResult(result) {
    ...
    layer.querySelector(".minigame-title").innerHTML = `<b>本次采矿结束</b><span>${summary}</span>`;
